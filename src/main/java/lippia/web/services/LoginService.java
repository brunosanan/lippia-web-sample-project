...
public class LoginService{
    ...
    public static void inputDatos(String mail, String password){
        if (mail.isEmpty()) {
            WebActionManager.setInput(LoginConstants.MAIL_INPUT, "x\b");
        }
        else{
            WebActionManager.setInput(LoginConstants.MAIL_INPUT, mail);
        }
        if (password.isEmpty()) {
            WebActionManager.setInput(LoginConstants.PASSWORD_INPUT, "x\b");
        }
        else{
    ...
    public static void verifyErrorMessage(String error, String locator){
        //WebActionManager.isPresent(WebActionManager.getText(xpath));
        Assert.assertEquals(error, WebActionManager.getText(locator));
    }
