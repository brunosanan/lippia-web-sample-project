package lippia.web.constants;

public class TrackerConstants {
    public static final String WORKSPACE_SELECTOR = "xpath://img[@alt='Workspace dropdown options']";

    public static final String WORKSPACE_MANAGE = "xpath://a[contains(text(), 'Manage')]";

    public static final String AVATAR = "xpath://avatar";

    public static final String LOGOUT_BUTTON = "xpath://a[normalize-space()='Log out']";

    public static final String PROJECT_BUTTON = "xpath://img[@alt='View Projects']";
}
